...
# With project-scoped directories, archiving moves tasks/{slug}/ as a unit.
# Only preserve stable entries at tasks/ root.
ARCHIVE_PRESERVE = {
    "archive",
    ".gitkeep",
}
...
def _derive_archive_name(state: OrchestratorState) -> str:
    """Build archive folder name from the project slug and current UTC date."""
    date_str = datetime.now(timezone.utc).strftime("%Y%m%d")
    return f"{state.project_slug}-{date_str}"


def archive_completed_project(state: OrchestratorState) -> bool:
    """Archive a completed project by moving tasks/{slug}/ into tasks/archive/.
    ...
    if not state.project_slug:
        logger.warning("Cannot archive completed project without a project slug")
        return False
    if state.project_slug in ARCHIVE_PRESERVE:
        logger.warning(
            "Refusing to archive preserved tasks entry '%s'",
            state.project_slug,
        )
        return False

    src_dir = _project_tasks_dir(state.project_dir, state.project_slug)
    if not src_dir.is_dir():
        logger.warning("No project directory to archive at %s", src_dir)
        return False

    archive_root = tasks_dir / "archive"
    archive_root.mkdir(parents=True, exist_ok=True)

    base_name = _derive_archive_name(state)
    archive_dir = archive_root / base_name
    collision_count = 2
    while archive_dir.exists():
        archive_dir = archive_root / f"{base_name}-{collision_count}"
        collision_count += 1

    try:
        shutil.move(str(src_dir), str(archive_dir))
        logger.info("Archived project directory %s -> %s", src_dir, archive_dir)
    except OSError as exc:
        logger.warning(
            "Failed to archive project directory %s -> %s: %s",
            src_dir,
            archive_dir,
            exc,
        )
